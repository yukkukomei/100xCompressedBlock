...
public class CompressedItem {
    ...
    static {
        for (int i = 0; i < 100; i++) {
            final int index = i;
            COMPRESSED_DIAMOND_BLOCK_ITEMS[i] = ITEMS.register(COMPRESSED_DIAMOND_BLOCK_ITEM + (index + 1),
                    () -> new BlockItem(CompressedBlock.COMPRESSED_DIAMOND_BLOCKS[index].get(), new Item.Properties()));
        }
        for (int i = 0; i < 100; i++) {
            final int index = i;
            COMPRESSED_IRON_BLOCK_ITEMS[i] = ITEMS.register(COMPRESSED_IRON_BLOCK_ITEM + (index + 1),
                    () -> new BlockItem(CompressedBlock.COMPRESSED_IRON_BLOCKS[index].get(), new Item.Properties()));
        }
    }
}
